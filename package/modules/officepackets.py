...
class OfficePackets:
    ...
    def get_status_msword(self):
        ...
        try:
            # Простая проверка через поток, если он существует
            if hasattr(self, '__msword_thread') and self.__msword_thread:
                # Проверяем, есть ли у потока объект Word и работает ли он
                if hasattr(self.__msword_thread, '_MsWordThread__word') and self.__msword_thread._MsWordThread__word:
                    try:
                        # Пробуем получить версию Word для проверки соединения
                        _ = self.__msword_thread._MsWordThread__word.Version
                        return True
                    except Exception:
                        # Если не удалось получить версию, значит соединение потеряно
                        self.__status_msword = False
                        if self.__osbm.obj_stab.get_is_active():
